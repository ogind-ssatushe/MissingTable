...
public final class TableNameParser {
	...
	public TableNameParser(final String sql) {
		...
		if (isSpecialDelete(firstToken, tokens, index)) {
			handleSpecialDelete(firstToken, tokens, index);
	...
	private String clean(final String normalized) {
		int start = normalized.indexOf(TOKEN_START);
		int end = NO_INDEX;
		if (start != NO_INDEX) {
			end = normalized.indexOf(TOKEN_END);
			if (end != NO_INDEX) {
	...
	private boolean isSpecialDelete(final String currentToken, final String[] tokens, int index) {
	...
	private void handleSpecialDelete(final String currentToken, final String[] tokens, int index) {
